...
type VehicleStateData struct {
	...
	VehFault        []uint32 `json:"vehFault"`        // 车辆故障状态 按位定义，返回按位掩码整数数组（Integer[]），若未上报返回空
	Doors           []uint32 `json:"doors"`           // 车门状态 按位定义，返回按位掩码整数数组（Integer[]），若未上报返回空
}

type VehicleStateReq struct {
	VehicleId    string `json:"vehicleId,optional"`    // 车辆编号，非必须
	CategoryCode int    `json:"categoryCode,optional"` // 车辆类型编码，非必须
}

type VehicleStateResp struct {
